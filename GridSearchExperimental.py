...
class GridSearch():
    ...
    def grid_search(self, X, y, X_ts = None, y_ts = None):
        ...
        is_nn = isinstance(self.pipeline.final_element, NeuralNetwork)
        ...
        n_epochs = 1
        if(is_nn):
            nn                  = self.pipeline.final_element
            n_epochs            = nn.max_epochs
            use_early_stopping  = nn.use_early_stopping
            self.pipeline.final_element.use_early_stopping = False
            self.pipeline.final_element.mode = 'standard'
        
        n_combinations = len(self.params_combinations)
        
        self.vl_scores_matrix_full = np.zeros(shape = (n_fold, n_combinations, n_epochs))
        self.tr_scores_matrix_full = np.zeros(shape = (n_fold, n_combinations, n_epochs))
        ...
        for tr_indexes, vl_indexes in folds:
            ...
            print("params_combinations ", n_combinations)
            for i, combination in enumerate(self.params_combinations):
                ...
                #[epoch, :, combination]
                if(is_nn):
                    self.vl_scores_matrix_full[n_iter][:][i] += pipeline_copy.final_element.vl_acc
                    self.tr_scores_matrix_full[n_iter][:][i] += pipeline_copy.final_element.tr_acc
                else:
                    self.vl_scores_matrix_full[n_iter][:][i] += pipeline_copy.score(X_vl, y_vl)
                    self.tr_scores_matrix_full[n_iter][:][i] += pipeline_copy.score(X_tr, y_tr)
        ...
        #2D matrices where on (x,y) you have the mean score of combination x
        #at iteration y
        self.vl_scores_matrix_full_mean = self.vl_scores_matrix_full.mean(0)
        self.tr_scores_matrix_full_mean = self.tr_scores_matrix_full.mean(0)
        
        self.vl_scores_matrix_full_std = self.vl_scores_matrix_full.std(0)
        self.tr_scores_matrix_full_std = self.tr_scores_matrix_full.std(0)
        
        #TODO: selection without early stopping (instead than taking argmax of
        #the whole matrix, it should take just the argmax of the last column.)
        #best_score_index = [best combination, best epoch] 
        if(self.is_regression_task):
            self.best_score_index = np.unravel_index(np.argmin(self.vl_scores_matrix_full_mean, axis=None), 
                                                     self.vl_scores_matrix_full_mean.shape)
        else:
            self.best_score_index = np.unravel_index(np.argmax(self.vl_scores_matrix_full_mean, axis=None),
                                                     self.vl_scores_matrix_full_mean.shape)
        
        
        self.best_combination_index = self.best_score_index[0]
        
        #vl_scores[i]: best validation score returned by combination i
        #vl_score_indexes[i]: epoch where the combination i did the best validation score
        if(self.is_regression_task):
            self.vl_scores = np.amin(self.vl_scores_matrix_full_mean, axis=1)
            self.vl_score_indexes = self.vl_scores_matrix_full_mean.argmin(axis=1)
        else:
            self.vl_scores = np.amax(self.vl_scores_matrix_full_mean, axis=1)
            self.vl_score_indexes = self.vl_scores_matrix_full_mean.argmax(axis=1)
        
            
        #standard deviation vl score, (mean/standard deviation) of the training
        #score for each combination i at the iteration when the validation score
        #was minimized (or maximized for classification tasks)
        self.vl_scores_std = self.vl_scores_matrix_full_std[range(len(self.vl_score_indexes)),self.vl_score_indexes]
        self.tr_scores     = self.tr_scores_matrix_full_mean[range(len(self.vl_score_indexes)),self.vl_score_indexes]
        self.tr_scores_std = self.tr_scores_matrix_full_std[range(len(self.vl_score_indexes)),self.vl_score_indexes]
        ...
        if(is_nn):
            if(use_early_stopping):
                n_iter_totale = self.best_score_index[1]
