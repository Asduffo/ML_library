# -*- coding: utf-8 -*-
"""
Runs the test described in section 4.2 and plots figure 4.1
...
from BaseOptimizer import Adam
import pandas as pd
...
nn.optimizer.learning_rate = .075

nn.max_epochs = 9000000
nn.mode = 'gradient'
...
#uncomment in case you want to run the full grid search (it might take a while)

"""
###############################################################################
#full grid search (decomment to run)

#L2 regularization should stay behind 0.00002 (meaning: must be equal or smaller)
#maybe 0.00003

params = {
    'fixed': [
        {
            # 'nn__optimizer__learning_rate' : [.1, .095, .09, .085, .08, .075, .07, .065],
        }
    ],
    'variable': []
}
